...
new_working_directory = "D:\Personal\BHTstudy\software project\PracticeCopy_BHT_advanced_software_engineering_WiSe23-main\DeepQLearn_FlappyBird"
...
def test(opt):
    ...
    if torch.cuda.is_available():
        model = torch.load("{}/flappy_bird_100000".format(opt.saved_path))
    else:
        model = torch.load("{}/flappy_bird_100000".format(opt.saved_path), map_location=lambda storage, loc: storage)
    model.eval()
